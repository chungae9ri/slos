...
void init_pgt_s(void)
{
	 ...
	 */
	ppage_dir = (unsigned int *)KERN_PGD_START_BASE_S;
	...
	pcur = (unsigned int *)KERN_PGT_START_BASE_S;
	for (i = 0; i < 4; i++) { 
		for (j = 0; j < 1024; j++) {
			/* 
			 * 0x1E1 is
	 ...
	 * for page_directory : 4K entry 
	 */
	ppage_tbl = (unsigned int *)KERN_PGT_START_BASE_S;
	...
	/* 
	 *  remap page table for kernel heap area start at 14MiB size 1MiB (0xC0E00000 ~ 0xC0F00000).
	 */
	for (i = 0; i < 1 * 256; i++) {
		ppage_tbl[0xC0E00 + i] = 0;
	}

	return;
}
void init_pgt_ns(void)
{
	unsigned int i, j;
	unsigned int *pcur, *ppage_dir, *ppage_tbl;
	 ...
	 * for page_directory : 4K entry 
	 */
	ppage_dir = (unsigned int *)KERN_PGD_START_BASE_NS;
	 ...
	 * ? ~ ?MB: ramdisk img
	 */			
	pcur = (unsigned int *)KERN_PGT_START_BASE_NS;
	for (i = 0; i < 4; i++) { 
		for (j = 0; j < 1024; j++) {
			/* 
			 * 0x1E9 is
	 ...
	 * for page_directory : 4K entry 
	 */
	ppage_tbl = (unsigned int *)KERN_PGT_START_BASE_NS;
		...
		struct vmpool *kheap)
{
	...
	init_framepool(kfp, KERN_FRAME_START, KERN_FRAME_NUM); 
