...
void init_pagetable(struct pagetable *ppagetable, PG_TYPE pagetype) 
{
	...
	if(pagetype == PG_TABLE_KERN) {
		/* 0x83A
		 * Bit[1:0] = 10 : 2b00 : fault, 2b01 : large page, 2b1x : small page 
		 * Bit[2] = 0 : Not Bufferable
		 * Bit[3] = 1 : Cacheable
		 * Bit[5:4] = 11 : AP[1:0] Access permission R/W at full access
		 * Bit[8:6] = 000 : TEX[2:0] = b000, C:1, B:0 : write through, no write allocate
		 * Bit[9] = 0 : AP[2]
		 * Bit[10] = 0 : not shareable
		 * Bit[11] = 1 : not global
		 */
