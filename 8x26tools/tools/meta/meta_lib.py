...
-------------------------------------------------------------------------------
  ...
  $Header: //source/qcom/qct/tools/meta/00/01/common/tools/meta/meta_lib.py#35 $
  $DateTime: 2013/11/19 22:08:48 $
...
Attributes, if any, are not returned.'''
   ...
   def __get_new_path (self, build_id):
      ...
      try:
         #Try opening Findbuild with an assumption that it is existing in the environment
         self.log("\nFindBuild"+" "+build_id)  #Debug Statement - Print FindBuild Command
         stuff = subprocess.Popen(['FindBuild', build_id], stderr=subprocess.STDOUT, stdout=subprocess.PIPE).stdout.readlines()
         self.log("FindBuild returned:"+' '.join(stuff)+"\n")  #Debug Statement - Print FindBuild Output
         if len(stuff) == 0:
            raise
      except:
         self.log("FindBuild could not be found in environment.")
         try:
            if self.on_linux:
               self.log("/prj/qct/asw/qctss/linux/bin/FindBuild"+" "+ build_id) #Debug Statement - Print FindBuild Command
               stuff = subprocess.Popen(['/prj/qct/asw/qctss/linux/bin/FindBuild', build_id],stderr=subprocess.STDOUT, stdout=subprocess.PIPE).stdout.readlines()
               self.log("FindBuild returned:"+' '.join(stuff)+"\n") #Debug Statement - Print FindBuild Output
            else:
               self.log("\\\\stone\\aswcrm\\smart\\nt\\bin\\FindBuild"+" "+ build_id) #Debug Statement - Print FindBuild Command
               stuff = subprocess.Popen(['\\\\stone\\aswcrm\\smart\\nt\\bin\\FindBuild', build_id],stderr=subprocess.STDOUT, stdout=subprocess.PIPE).stdout.readlines()
               self.log("FindBuild returned:"+' '.join(stuff)+"\n") #Debug Statement - Print FindBuild Output
         except:
            self.log("FindBuild could not be found.")
