...
#=============================================================================
#
#  $Header: //source/qcom/qct/core/securemsm/Tools/main/latest/gensecimage/qpsa/qpsa.py#13 $
#  $DateTime: 2013/11/04 22:15:45 $
#  $Author: yliong $
#  $Change: 4716092 $
...
def generate_root_ca_files(rootcakey_file='qpsa_rootca.key', rootcacert_file='qpsa_rootca.crt'):
    ...
    self_signed_cert(keyfile="qpsa_rootca.key", outfile=rootcacert_file, subj='/C=US/ST=California/L=San Diego/OU=General Use Test Key (for testing only)/OU=CDMA Technologies/O=QUALCOMM/CN=QCT Root CA 1')
...
def generate_attestation_ca_files(attestcakey_file='"qpsa_attestca.key"', attestcacert_file='qpsa_rootca.crt'):
    ...
    gen_signing_request(keyfile="qpsa_attestca.key", outfile="qpsa_attestca.csr", subj='/C=US/ST=CA/L=San Diego/OU=CDMA Technologies/O=QUALCOMM/CN=QUALCOMM Attestation CA')
...
if check_file_exists("qpsa.zip") == True:
    ...
    openssl_configfiles = ["opensslroot.cfg", "v3_attest.ext", "QcomDevSigningToolDefaults.xml", "opensslroot.cfg"]
    ...
    # Check if cert/key are specified
    isFileMissing = False
    if (rootcakey is None or check_file_exists(rootcakey) == False):
        LOG.write("\n\nqpsa.zip does not exist and rootcakey parameter is not specified.\n\n")
        print "\n\nERROR:qpsa.zip does not exist and rootcakey parameter is not specified.\n\n"
        isFileMissing = True
    elif rootcacert is None or check_file_exists(rootcacert) == False:
        LOG.write("\n\nqpsa.zip does not exist and rootcacert parameter is not specified.\n\n")
        print "\n\nERROR:qpsa.zip does not exist and rootcacert parameter is not specified.\n\n"
        isFileMissing = True
    elif attestcakey is None or check_file_exists(attestcakey) == False:
        LOG.write("\n\nqpsa.zip does not exist and attestcakey parameter is not specified.\n\n")
        print "\n\nERROR:qpsa.zip does not exist and attestcakey parameter is not specified.\n\n"
        isFileMissing = True
    elif attestcacert is None or check_file_exists(attestcacert) == False:
        LOG.write("\n\nqpsa.zip does not exist and attestcacert parameter is not specified.\n\n")
        print "\n\nERROR:qpsa.zip does not exist and attestcacert parameter is not specified.\n\n"
        isFileMissing = True

    if isFileMissing is True:
        LOG.close()
        sys.exit(1)
