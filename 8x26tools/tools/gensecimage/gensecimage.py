...
#-------------------------------------------------------------------------------
#
#  $Header: //source/qcom/qct/core/securemsm/Tools/main/latest/gensecimage/gensecimage.py#35 $
#  $DateTime: 2013/11/25 22:54:20 $
#  $Author: yliong $
...
def create_file_to_sign(source_file_name, target_file_name, mbn_header_size, size_of_preamble, cert_chain_size_in = CERT_CHAIN_ONEROOT_MAXSIZE):
  ...
  bin_data = source_fp.read(orig_image_size)
...
def read_ini_file(section_name):
  ...
  if (section_name == 'appsbl'):
     if (config.has_option(section_name,'file_name') == True):
        source_image = target_config + source_image
...
def parse_args_and_validate(args):
    ...
    global postsignconfig_filename
    global key_config
#LGE_CHANGE_S add option for target dir, fred.cho@lge.com
...
#       opts, remainder = getopt.getopt(args, "hv", ["help", "stage=", "section=", "config=", "signconfig=", "postsignconfig=", "encrypt_keys="])
       opts, remainder = getopt.getopt(args, "hv", ["help", "stage=", "section=", "config=", "signconfig=", "postsignconfig=", "encrypt_keys=", "target="])
#LGE_CHANGE_E
...
#LGE_CHANGE_S add option for target dir, fred.cho@lge.com
         ...
         if (target_config == ''):
            target_config = arg
            log("Found argument for target %s" % (target_config))
         else:
            log("Can't have multiple target!")
#LGE_CHANGE_E
      ...
      if filelock_enable is False or lock.acquire():
          try:
            ...
            for each_section in section_list:
                ...
                if ((stage == "postsign") or (stage== "qpsa:sign") or (stage== "encryptonly")):
